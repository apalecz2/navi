...
| **Briefing** | 60s (composes ahead of `BRIEFING_AT`, sends at `BRIEFING_AT`, evaluates response after grace) | Composes and sends the morning briefing, tracks whether it was answered — P3.5, session 19, see [11-goals-spec.md](11-goals-spec.md#the-morning-briefing) |
...
briefing loop, ~30 min before BRIEFING_AT
  ...
  → store the text in kv.briefing_pending; on generation failure, or with no
      model client at all, store a plain template built from the same context

briefing loop, at BRIEFING_AT
  → send kv.briefing_pending via the conversation transport; if it is absent
      (a restart spanning the compose window), render the template
      synchronously — no model call at or after the send
  → one tx: conversations row with context_ref = briefing:{date};
      kv.last_briefing_date = {date}; kv.briefing_awaiting_response = {date}\t{sent_at};
      delete kv.briefing_pending

briefing loop, every tick:
  → if kv.briefing_awaiting_response is set and any inbound message exists
      after sent_at, clear it (answered)
  → else if the grace window has closed, record unanswered and clear it
```
...
occurrence table. "Unanswered" writes no `missed` and no transition; it is a
counter (`navi_briefing_unanswered_total`) and a log line, read later by the
day's tone strategy. See [11-goals-spec.md](11-goals-spec.md#the-morning-briefing).
...
Scattered across six loops and a dozen handlers it is an audit. The boundary
...
| Briefing generation fails (P3.5, session 19) | Morning message is a plain template, not personalized | Same fallback rule as N3 and the copywriter; `composeBriefing` computes the template first and replaces it only on a usable completion, and a missed compose window templates synchronously at send |
