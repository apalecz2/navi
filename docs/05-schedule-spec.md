...
- **`from_date`'s boundary holds for that edit, not forever.** The floor is applied
  to the materialization pass the edit runs, and nothing persists it: `items.schedule`
  is one flat value with no record that a boundary was ever asked for. A later
  unbounded pass — the nightly run, the sweeper's horizon backfill, or the next edit —
  re-plans the whole future window against the current schedule, and pre-boundary
  pending rows that the new schedule would not produce are deleted then. History is
  still safe (the delete guard excludes every non-pending row); what is not durable
  is the *pending* tail before the boundary. Fixing this means per-date schedule
  versioning, which the flat-column design deliberately does not have.
